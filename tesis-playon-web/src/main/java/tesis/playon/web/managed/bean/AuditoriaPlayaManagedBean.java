...
public class AuditoriaPlayaManagedBean implements Serializable {
	...
	private EstadoPlaya estadoPendiente, estadoAprobada, estadoRechazada,
			estadoDeBaja;
	...
	@PostConstruct
	private void init() {
		estadoPendiente = getEstadoPlayaService().findByNombreEstadoPlaya(
				"Pendiente");
		playasPendientesList = getPlayaService().findByEstado(estadoPendiente);

		estadoAprobada = getEstadoPlayaService().findByNombreEstadoPlaya(
				"Aprobada");
		playasAprobadasList = getPlayaService().findByEstado(estadoAprobada);

		estadoRechazada = getEstadoPlayaService().findByNombreEstadoPlaya(
				"Rechazada");
		playasRechazadasList = getPlayaService().findByEstado(estadoRechazada);

		estadoDeBaja = getEstadoPlayaService().findByNombreEstadoPlaya(
				"De Baja");
	...
	public void rejectPlayaAuditoria() {
		try {
			...
			getPlayaService().update(playaSeleccionada);
			if (!StringUtils.isEmpty(playaSeleccionada.getEmail())
					|| playaSeleccionada.getEmail() != null) {
			...
			estadoPendiente = getEstadoPlayaService().findByNombreEstadoPlaya(
					"Pendiente");
			playasPendientesList = getPlayaService().findByEstado(
					estadoPendiente);

			estadoRechazada = getEstadoPlayaService().findByNombreEstadoPlaya(
					"Rechazada");
			playasRechazadasList = getPlayaService().findByEstado(
					estadoRechazada);
	...
	public void approvePlayaAuditoria() {
		try {
			...
			estadoAprobada = getEstadoPlayaService().findByNombreEstadoPlaya(
					"Aprobada");
			playasAprobadasList = getPlayaService()
					.findByEstado(estadoAprobada);
	...
	public SelectItem[] getBarriosOptions() {
		if (barriosOptions == null) {
			...
			for (int i = 0; i < barrios.size(); i++) {
				barriosOptions[i + 1] = new SelectItem(barrios.get(i), barrios
						.get(i).getNombre());
			}
		}
	...
	public SelectItem[] getEstadosOptions() {
		if (estadosOptions == null) {
			...
			for (int i = 0; i < estados.size(); i++) {
				estadosOptions[i + 1] = new SelectItem(estados.get(i), estados
						.get(i).getNombre());
			}
		}
