...
public class DetalleEstadiaManagedBean implements Serializable {
	...
	private void init() {
		...
		try {
			...
			if (usuarioLoggeado != null) {
				empleadoLoggeado = getEmpleadoService().findByUsuario(
						usuarioLoggeado);
				playaLoggeada = usuarioLoggeado.getPlaya();
				if (playaLoggeada != null) {
					cuentaPlaya = getCuentaPlayaService().findByPlaya(
							playaLoggeada);
					if (cuentaPlaya == null) {
	...
	public void searchVehiculo() {
		...
		try {
			...
			if (vehiculo != null) {
				...
				detalleEstadia = getDetalleEstadiaService()
						.findByVehiculoDetalleEstadia(vehiculo);
				if (detalleEstadia != null && !detalleEstadia.getCobrado()) {
					...
					horaIngreso = Time.valueOf(hour + ":" + minutes + ":"
							+ seconds);

				}
				tarifaPlayaList = getTarifaService()
						.findTarifaVigenteByPlayaAndCategoriaVehiculo(
								playaLoggeada,
	...
	public String registrarIngresoAbono() {
		try {
			...
			Timestamp fechaHoraIngreso = new Timestamp(Calendar.getInstance()
					.getTimeInMillis());
			detalleEstadia = new DetalleEstadia(estadia, vehiculo,
					empleadoLoggeado, fechaHoraIngreso, false);
			...
			FacesMessage message = new FacesMessage(
					...
					"Se registró el ingreso exitosamente del vehiculo con abono mensual con patente: "
							+ patente, null);
			FacesContext.getCurrentInstance().addMessage(null, message);
	...
	public String registrarEgresoAbonado() {
		try {
			Timestamp fechaHoraEgreso = new Timestamp(Calendar.getInstance()
					.getTimeInMillis());
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Se registró el egreso exitosamente del vehículo con abono patente: "
							+ patente, null);
			FacesContext.getCurrentInstance().addMessage(null, message);
	...
	public String registrarIngresoVehiculo() {
		try {
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Se registró el ingreso exitosamente del vehiculo patente: "
							+ patente, null);
			FacesContext.getCurrentInstance().addMessage(null, message);
	...
	public void calcularImporte() {
		...
		if ("Por Hora".equals(tipoEstadia.getNombre())) {
			fechaHoraEgreso = new Timestamp(Calendar.getInstance()
					.getTimeInMillis());
			detalleEstadia.setFechaHoraEgreso(fechaHoraEgreso);
			...
			if (detalleEstadia.getPromocion() != null) {
				importe = importe
						* (1 - detalleEstadia.getPromocion().getDescuento() / 100);
			}
		} else if ("Por Mes".equals(tipoEstadia.getNombre())) {
			Calendar calendario = Calendar.getInstance();
			calendario.clear();
			calendario.setTimeInMillis(detalleEstadia.getFechaHoraIngreso()
					.getTime());
	...
	public String registrarEgresoVehiculo() {
		...
		try {
			...
			TipoPago tipoPagoEfectivo = getTipoPagoService()
					.findByNombreTipoPago("Contado");
			...
			if (saldoActual < importe) {
				...
				transaccionCliente = new TransaccionCliente(
						detalleEstadia.getFechaHoraEgreso(), saldoActual,
				...
				transaccionCliente = new TransaccionCliente(
						detalleEstadia.getFechaHoraEgreso(), saldo,
				...
				transaccionCliente = new TransaccionCliente(
						detalleEstadia.getFechaHoraEgreso(), -importe,
						tipoPagoCuenta, cuentaCliente);
				getTransaccionClienteService().save(transaccionCliente);
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Se registró el egreso exitosamente del vehículo patente: "
							+ patente, null);
			FacesContext.getCurrentInstance().addMessage(null, message);
	...
	public void handleTarifa() {
		if (tarifaSeleccionada != null && !tarifaSeleccionada.equals("")) {
			...
			promocionesDisponibles = getPromocionService()
					.findByTipoEstadiaAndPlaya(tipo, playaLoggeada);
		} else
	...
	public String getHoraActual() {
		String horaActual = new SimpleDateFormat("HH:mm:ss").format(Calendar
				.getInstance().getTime());
		return horaActual;
	}
	...
	public StreamedContent getFile() {
		try {
			...
			titulo.add(new Paragraph("Comprobante de ingreso emitido el: "
			...
			titulo.add(new Paragraph(
					"Hora de ingreso: "
							+ "una vez que el archivo descargue, poner fecha de egreso aca!",
					fuenteCuerpo));
			titulo.add(new Paragraph("Patente: "
					+ getDetalleEstadia().getVehiculo().getPatente(),
					fuenteCuerpo));
			titulo.add(new Paragraph("Modelo: "
					+ getDetalleEstadia().getVehiculo().getModeloVehiculo()
							.getNombre(), fuenteCuerpo));
			titulo.add(new Paragraph("Importe: "
					+ getDetalleEstadia().getImporteTotal(), fuenteCuerpo));
			titulo.add(new Paragraph("Saldo: "
					+ getDetalleEstadia().getVehiculo().getCliente()
							.getCuentaCliente().getSaldo(), fuenteCuerpo));

			String sep = File.separator;
			ExternalContext extContext = FacesContext.getCurrentInstance()
					.getExternalContext();
			String logo = extContext.getRealPath("resources" + sep + "images"
					+ sep + "transacciones.png");
			/************************************/
			doc.open();
