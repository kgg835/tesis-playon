...
public class PlayaManagedBean implements Serializable {
	...
	public String addPlayaAdmin() {
		try {
			...
			FacesMessage message = new FacesMessage(
					FacesMessage.SEVERITY_ERROR, "Error, no se pudo agregar: "
			...
			FacesMessage message = new FacesMessage(
					...
					"Error, no se pudo agregar la Playa. Nombre de usuario o mail Duplicados",
					"Usuario duplicado");
			FacesContext.getCurrentInstance().addMessage(null, message);
		}
		return ERROR;
	...
	public String addSolicitudPlaya() {
		try {
			...
			estado = getEstadoPlayaService().findByNombreEstadoPlaya(
					"Pendiente");

			Usuario usuario = addUsuario();

			CargoEmpleado cargo = getCargoEmpleadoService().findByNombreCargo(
					"Gerente General");
			...
			RolesPorUsuario rp = new RolesPorUsuario(usuario.getNombreUser(),
					"ROLE_PLAYA_GERENTE");
	...
	public void deletePlayaAdmin(Playa playa) {
		try {
			EstadoPlaya estado = getEstadoPlayaService()
					.findByNombreEstadoPlaya("De Baja");
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Se dió de baja la playa: " + playa.getNombreComercial(),
					"");
			FacesContext.getCurrentInstance().addMessage(null, message);
		} catch (Exception e) {
			FacesMessage message = new FacesMessage(
					FacesMessage.SEVERITY_ERROR,
					"Error, no se pudo dar de baja la playa: "
							+ playa.getNombreComercial(),
					"Por favor, inténtelo más tarde.");
			FacesContext.getCurrentInstance().addMessage(null, message);
		}
	...
	// NO USAR. HAY Q CORREGIRLE ALGUNAS COSAS ---> TOMAR COMO REFERENCIA LA DE
	// ARRIBA
	...
	public String updatePlayaAdmin() {
		try {
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
					playaSelected.getNombreComercial()
	...
	public void findPlayaById() {
		...
		int idPlayaSelected = Integer.parseInt(facesContext
				.getExternalContext().getRequestParameterMap().get("id"));
		PlayaManagedBean.playaSelected = getPlayaService().findById(
				idPlayaSelected);
	}
	...
	public void listadoPlayasPDF(Object document) throws IOException,
