...
public class TransaccionPlayaManagedBean implements Serializable {
	...
	public List<String> complete(String query) {
		...
		List<Playa> playas = this.getPlayaService().findByLikeNombreComercial(
				query);
	...
	public List<TransaccionPlaya> updateTransaccionesPorFechaList() {
		...
		transaccionesPorFechaList = getTransaccionPlayaService()
				.findTransaccionesByFecha(cuentaPlaya, fechaDesde, fechaHasta);
	...
	public void setTransaccionesPorFechaList(
			List<TransaccionPlaya> transaccionesPorFechaList) {
	...
	public SelectItem[] getTipoPagoOptions() {
		if (tipoPagoOptions == null) {
			...
			for (int i = 0; i < tiposPago.size(); i++) {
				tipoPagoOptions[i + 1] = new SelectItem(tiposPago.get(i),
						tiposPago.get(i).getNombre());
			}
		}
