...
public class PlayaDao implements IPlayaDao {
	...
	public Playa findByNombreComercial(String nombreComercial) {
		List<?> list = getSessionFactory()
				.getCurrentSession()
				.createQuery(
						"from Playa where nombreComercial=?  order by razonSocial")
	...
	public List<Playa> findByLikeNombreComercial(String nombreComercial) {
		...
		List<?> list = getSessionFactory()
				.getCurrentSession()
				.createQuery(
						"from Playa where nombreComercial LIKE ? order by nombreComercial")
				.setParameter(0, "%" + nombreComercial + "%").list();
		if (!list.isEmpty()) {
	...
	public Playa findByRazonSocial(String razonSocial) {
		List<?> list = getSessionFactory()
				.getCurrentSession()
				.createQuery(
						"from Playa where razonSocial=?  order by razonSocial")
	...
	public Playa findById(int idPlaya) {
		List<?> list = getSessionFactory().getCurrentSession()
				.createQuery("from Playa where id=?").setParameter(0, idPlaya)
				.list();
		if (!list.isEmpty())
			return (Playa) list.get(0);
		return null;
	...
	public List<Playa> findPlayasCercanas(Double longitud, Double latitud,
			int distancia) {
		List<?> list = getSessionFactory().getCurrentSession()
				.getNamedQuery("callPlayasStoreProcedure")
	...
	public List<Playa> findAll() {
		...
		List<?> list = getSessionFactory().getCurrentSession()
				.createQuery("from Playa order by razonSocial").list();
		if (!list.isEmpty()) {
			for (Object object : list) {
	...
	public List<Playa> findByEstado(EstadoPlaya estado) {
		List<Playa> playa = new ArrayList<Playa>();
		List<?> list = getSessionFactory().getCurrentSession()
				.createQuery("from Playa where estado=?")
	...
	public boolean existeEmail(String email) {
		List<?> list = getSessionFactory().getCurrentSession()
				.createQuery("from Playa where email=?").setParameter(0, email)
				.list();
		if (list.isEmpty()) {
			return false;
		}
